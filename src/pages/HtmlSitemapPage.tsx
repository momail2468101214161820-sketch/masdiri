...
const HtmlSitemapPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>خريطة مصدري | مصدري</title>
        <meta name="description" content="خريطة مصدري: روابط للأقسام والمحافظات وأحدث الأخبار." />
        <link rel="canonical" href={`${SITE_URL}/sitemap`} />
        <meta name="robots" content="index, follow" />
      </Helmet>
      <SiteHeader />
      <Breadcrumbs items={[{ label: "خريطة مصدري" }]} />
      <main className="container py-6 space-y-10">
        <header>
          <h1 className="newspaper-heading text-3xl border-b-2 border-foreground pb-3 mb-4">خريطة مصدري</h1>
          <form onSubmit={onSearch} className="flex gap-2 max-w-md">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ابحث في مصدري…" className="flex-1 border border-border rounded-lg px-3 py-2 text-sm bg-background" />
