...
const SYSTEM_PROMPT = `أنت «بوت مصدري» — المساعد الرسمي الذكي لـ "مصدري للأخبار المصرية والعالمية".
...
async function buildLocalReply(query: string) {
  ...
  if (isAboutQuery(q)) {
    return `أنا **بوت مصدري** — المساعد الرسمي لمؤسسة "مصدري للأخبار المصرية والعالمية"، بقيادة **برئاسة وتطوير: البشمبرمج/ خالد عاطف عبدالحكيم** وتطوير **التقني/ خالد عاطف عبدالحكيم عويس**.`;
  }
  if (isResultQuery(q)) {
