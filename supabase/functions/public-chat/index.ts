// بوت "صوت البلد" — مساعد محادثة عام للزوار (Gemini عبر بوابة Lovable AI)
...
const SYSTEM_PROMPT = `أنت «بوت صوت البلد» — المساعد الرسمي الذكي لـ "صوت البلد — صوتك واصل".
...
- مهامك: التعريف بصوت البلد، تلخيص وشرح الأخبار بتفاصيلها الكاملة، الإجابة عن أسعار العملات/الذهب، الاستعلام عن نتيجة الإعدادية 2026 (برقم الجلوس أو الاسم)، إرشاد المستخدم إلى أقسام الموقع، الرد على استفسارات الإعلان والتواصل.
- للحجز الإعلاني والتواصل المباشر: برئاسة د/ محمد الحاجري — واتساب: +20 100 618 8795.
...
function isAboutQuery(t: string) {
  return /من انت|من أنت|عرفني|عرف نفسك|انت مين|إنت مين|من هو الحاجري|من خالد|مين انت/i.test(t);
}
...
async function buildLocalReply(query: string) {
  ...
  if (isGreeting(q)) {
    return `وعليكم السلام، أهلاً بكم في **صوت البلد**. كيف يمكنني خدمتكم؟\n\nيمكنكم سؤالي عن:\n- آخر الأخبار\n- نتيجة الإعدادية 2026 (برقم الجلوس)\n- أسعار العملات والذهب\n- التواصل والإعلان`;
  }
  if (isContactQuery(q)) {
    return `## التواصل مع صوت البلد\n\n- **رئيس المؤسسة:** برئاسة د/ محمد الحاجري\n- **واتساب الحجوزات والإعلانات:** [+20 100 618 8795](https://wa.me/201006188795)\n\nيسعدنا تواصلكم في أي وقت.`;
  }
  if (isAboutQuery(q)) {
    return `أنا **بوت صوت البلد** — المساعد الرسمي لمؤسسة "صوت البلد — صوتك واصل"، بقيادة **برئاسة د/ محمد الحاجري** وتطوير **التقني/ خالد عاطف عبدالحكيم عويس**.`;
  }
  if (isResultQuery(q)) {
